...
function DSGamemode_Purge::messageSent(%this, %minigame, %client, %text)
{
	if (strPos("", %char = getSubStr(%text, 0, 1)) != -1) //"$%"
	{
		%name = %client.getPlayerName();
		if (isObject(%client.character))
			%name = %client.character.name;
		%does = "says";
		%structure = '<color:ff4040>%1<color:ffffff> %3, %4\"%2\"';
		%text = getSubStr(%text, 1, strLen(%text) - 1);
		if (!DSPurgeKillers.isMember(%client))
		{
			Parent::messageSent(%this, %minigame, %client, %text);
			return;
		}
		if (%char $= "%")
		{
		...
		{
			%other = ClientGroup.getObject(%i);
			if (DSPurgeKillers.isMember(%other))
			{
				%range = %shout ? 100 : 24;
				%zRange = %shout ? 32 : 16;
			}
